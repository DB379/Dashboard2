...
class User
{
    ...
    // Register page -Ovo je samo za registraciju korisnika na register.php stranici
    ...
    // CSRF token regeneration - Ovaj se koristi za login i register stranice
    ...
    // Function to sanitize input GLOBAL
    ...
    // Ovo prikazuje na settings stranici aktivnosti korisnika, kad su se logirali i kad su se izlogirali
    ...
    // Function to get all users from the database - koristim za admin stranicu
    public function fetchTable()
    {
        try {
            $stmt = $this->db->prepare("SELECT id, username, email, profile_pic, joindate, level FROM accounts");
            $stmt->execute();
            return $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
    ...
    // Function to get the user's role - KORISTIM ZA DEBUGGING
    public function getUserRole($username)
    {
        ...
        // If role is above 3, default to 0
        if ($role === 0 || $role > 3) {
            return 0;
        }
    ...
    {
        $stmt = $this->db->prepare("UPDATE accounts SET username = :username, email = :email, level = :level WHERE id = :id");
    ...
    public function fetchUsersWithPagination($recordsPerPage, $offset) {
        $query = "SELECT * FROM accounts ORDER BY id LIMIT ? OFFSET ?";
        $stmt = $this->db->prepare($query);
        $stmt->execute([$recordsPerPage, $offset]);
        return $stmt->fetchAll();
    }

    public function getTotalPages($itemsPerPage, $table) {
    ...
    public function getPaginatedData($page, $itemsPerPage, $table) {
        $offset = ($page - 1) * $itemsPerPage;
        $stmt = $this->db->prepare("SELECT * FROM $table LIMIT :limit OFFSET :offset");
    ...
    public function logout()
    {
        ...
        if (isset($_SESSION['username'])) {
            ...
            // Clear session key and logged status in the database
            $stmt = $this->db->prepare("UPDATE accounts SET session_key = NULL, logged = 0 WHERE username = :username");
            $stmt->bindParam(':username', $username, PDO::PARAM_STR);
            $stmt->execute();
    
            // Update the login status with the logout time
